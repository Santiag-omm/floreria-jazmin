                            ...
                            @foreach($products->take(5) as $index => $product)
                                <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                    @if($product->coverImage())
                                        <img src="{{ $product->coverImage()->url }}" class="d-block w-100" style="height:260px; object-fit:cover;" alt="{{ $product->name }}" loading="lazy" decoding="async">
                                    @else
    ...
    @forelse($products as $product)
        <div class="col-6 col-md-4 col-lg-3">
            <div class="card border-0 shadow-sm h-100 product-card">
                @if($product->coverImage())
                    <img class="card-img-top" src="{{ $product->coverImage()->url }}" alt="{{ $product->name }}" loading="lazy" decoding="async">
                @else
